<template>
	<view style="position: relative;height: 100%;width:100%;" :style="{paddingLeft:paddingLeft+'rpx'}">
		<view style="position: absolute;left:50%;top:50%;transform: translate(-50%,-50%);"
			...
			<view class="card" style="z-index: 12;"
				:style="{width:width2+'rpx',height:height2+'rpx',left:left2+'rpx',top:top2+'rpx','border-radius':borderRadius+'rpx',backgroundColor:cardBgColor}"
				:class="{'animation':isAnimating,'shadowEffect':hasShadow,'boderEffect':hasBorder}">
				<slot name="secondCard"></slot>
			</view>
			<view class="card" style="z-index: 11;"
				:style="{width:width3+'rpx',height:height3+'rpx',left:left3+'rpx',top:top3+'rpx','border-radius':borderRadius+'rpx',backgroundColor:cardBgColor}"
				:class="{'animation':isAnimating,'shadowEffect':hasShadow,'boderEffect':hasBorder}">
				<slot name="thirdCard"></slot>
			</view>
			<view class="card" style="z-index: 10;"
				:style="{width:width4+'rpx',height:height4+'rpx',left:left4+'rpx',top:top4+'rpx','border-radius':borderRadius+'rpx',backgroundColor:cardBgColor,opacity:opacity4}"
				:class="{'animation':isAnimating,'shadowEffect':hasShadow,'boderEffect':hasBorder}">
	...
	export default {
		...
		methods: {
			...
			touchMove: function(e) {
				...
				var curTouch = e.touches[0];
				if (this.dragDirection == "all" || this.dragDirection == "horizontal") this.left = curTouch.clientX -
					this.startLeft;
				if (this.dragDirection == "all" || this.dragDirection == "vertical") this.top = curTouch.clientY - this
					.startTop;
